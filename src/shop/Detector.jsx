...
export default function Detector() {
    const {detector, setDetector } = useContext(MyContext);
    ...
    const fetchData = async () => {
        try {
            ...
            setDetector(response.data.products);
        } catch (error) {
            console.error(error);
        } finally {
            setLoading(false);
    ...
    return (
        <div id='productPage'>
            ...
            <div style={{ height: shot ? "100%" : "0", overflow: "hidden" }} className='shopBoxes'>
                {loading ? (
                    <h2>YÜKLƏNİR...</h2>
                ) : detector && detector.length > 0 ? (
                    detector.map((product) => (
                        <div className='shopBox' key={product.id}>
                            ...
                            <div className='shopIcons'>
                                ...
                                <div className='shopIcon'>
                                    <Link to={`/leakage_detector/${product.id}`}>
