...
function UpdateServiceHowWeWorks() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
        await axios.post(`http://localhost:8000/api/our_vision_mission/service_info/${id}`, formDataToSend, {
            headers: {
                'Content-Type': 'multipart/form-data',
            },
        });
        setMessage('Service updated successfully!');
        navigate('/our_vision_mission'); // Uğurlu yeniləmə halında yönləndirin
    } catch (error) {
  ...
  return (
    <>
      ...
      <div className="adminHero">
        <h2>Update Our Vision/Mission Service</h2>
        {message && <p>{message}</p>}
        {loading && <p>Loading...</p>}
